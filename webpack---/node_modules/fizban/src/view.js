...
function transformSceneRangesToOffsets(scene, rect, viewportSize, isHorizontal, absoluteOffsetContext, offsetTree) {
    ...
    if (typeof duration === 'string') {
        ...
        if (startRange || start ? .name) {
            ...
            const absoluteStartOffset = transformRangeToPosition({ ...startRange,
                offset: 0
            }, viewportSize, rect);
            const absoluteEndOffset = transformRangeToPosition({ ...startRange,
                offset: 100
            }, viewportSize, rect);
        ...
        if (endRange || end ? .name) {
            ...
            const absoluteStartOffset = transformRangeToPosition({ ...endRange,
                offset: 0
            }, viewportSize, rect);
            const absoluteEndOffset = transformRangeToPosition({ ...endRange,
                offset: 100
            }, viewportSize, rect);
...
export function getTransformedSceneGroup(scenes, root, viewportSize, isHorizontal, absoluteOffsetContext) {
    ...
    const transformedScenes = scenes.map(scene => ({
        ...transformSceneRangesToOffsets(
            scene, {
                start: elementLayoutStart,
                end: elementLayoutStart + size
            },
            viewportSize,
            isHorizontal,
