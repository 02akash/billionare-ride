...
export const panoramaClientFactory = options => {
    ...
    const factory = panoramaClient({
        baseParams,
        data
    }).use(pageParamsEnricherPlugin(pageParams)).use(topLevelPropsExtractorPlugin(EXTRA_BASE_PARAMS)).use(environmentPlugin()).use(batchPlugin(BATCH_THROTTLE_DURATION_MS)).use(sentryPlugin({
        sentry,
        sentryDsn,
        sentryMain,
        sentryTransport
