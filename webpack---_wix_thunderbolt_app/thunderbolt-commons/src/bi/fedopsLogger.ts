...
export const createFedopsLogger = ({
	...
	const isFedopsMutingExperimentOpen = isExperimentOpen('specs.thunderbolt.muteFedopsEvents', experiments)
	const shouldReportOnlyInPanorama = isExperimentOpen('specs.thunderbolt.stopSendingAppLoadedInFedops', experiments)
	...
	const shouldMuteEvent = (event: string, isInteraction: boolean, overrides?: any): boolean => {
		...
		} else if (!shouldEventAlwaysReportToBi(event, overrides, isInteraction)) {
			return isFedopsMutingExperimentOpen
		}
		return false
	}
