...
import {
    createWixcodeExports as createPublicApiExports
...
import {
    CurrentCartService
} from './current-cart-service/CurrentCartService';
...
function getControllerFactory(controllerInstances, controllerFactories, type) {
    ...
    if (controllerInstances === null || controllerInstances === void 0 ? void 0 : controllerInstances[type]) {
        ...
        ctrlFactory = __assign({
            factory: controllerInstances[type][controllerFunction]
        }, (sentryConf ? {
            sentryDSN: sentryConf.dsn,
            baseUrlKey: sentryConf.baseUrlsKey
        } : {}));
        // @ts-expect-error
...
export function createViewerScript(controllerFactories, options) {
    ...
    return withErrorReportingWrapping({
        initAppForPage: function(initParams, apis, namespaces, platformServices) {
            ...
            if (platformServices.essentials.experiments.enabled(SPECS.ShouldPreloadCurrentCart)) {
                context.currentCartService = new CurrentCartService({
                    context: context,
                    apis: apis
                });
        ...
        exports: createPublicApiExports({
            context: context,
            origin: 'wixcode'
        }),
    });
