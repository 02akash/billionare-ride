import { HostSdk, ViewMode } from '../types/host-sdk';
...
export const getResizer = (
  ...
  if (isMobile && hostSdk.getViewMode() === ViewMode.Editor) {
    return mobileEditorResizer(hostSdk, dimensions);
  }
  if (isMobile) {
...
const mobileEditorResizer = (
  hostSdk: HostSdk,
  dimensions: Dimensions,
): Resizer => {
  return {
    async expand() {
      const { width, height } = await hostSdk.getScreenSize();
      await hostSdk.resizeTo(width, height);
    },
    async collapse(withPopup: boolean = false) {
      const dimension = getCollapsedDimension(dimensions, withPopup);
      await hostSdk.resizeTo(dimension.width, dimension.height);
