...
import { CommunicationChannelProvider, withSounds } from '@wix/chat-web';
...
const MessagePopup = React.lazy(
  () =>
    import(
      /* webpackChunkName: "message-pop-up" */ '../MessagePopup/MessagePopup'
    ),
);

const MinimizedChat = React.lazy(
  () =>
    import(
      /* webpackChunkName: "minimized-widget" */ '../MinimizedChat/MinimizedChat'
    ),
);
...
const _ChatWidget: React.FunctionComponent<ChatWidgetProps> = () => {
  ...
  return (
    <div
      ...
      role="main"
    >
      <CommunicationChannelProvider communicationChannels={{}}>
        {isSdkLoaded && !appState.withQab ? (
          <MessagePopup isMembersChat={isMembersChat} />
        ) : null}
        {isInEditingExperience && <EditorWidget />}
        {collapseExpand.isExpanded && isSdkLoaded ? (
